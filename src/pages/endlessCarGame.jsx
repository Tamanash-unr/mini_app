...
const EndlessCarGame = () => {
  ...
  const endGame = useCallback(async () => {
    ...
    // Process traditional game reward
    ...
    try {
      const gameResults = {
        ...
        topSpeed: gameState.displaySpeed
      };

      // Simple reward system - earn coins based on score and speed
      const baseReward = Math.floor(gameResults.score / 5); // 1 coin per 5 points
      const speedBonus = Math.floor(gameResults.topSpeed / 2); // 1 coin per 2 speed units
      const timeBonus = Math.floor(gameResults.gameTime / 10000); // 1 coin per 10 seconds
      const totalReward = Math.max(baseReward + speedBonus + timeBonus, 15); // Minimum 15 coins

      dispatch(updateEarnedFromGame(totalReward));
      console.log('🎉 Endless car game reward processed:', totalReward, 'coins');
    } catch (error) {
      console.error('Failed to process tokenomics reward:', error);
    }

    if (animationFrameRef.current) {
      cancelAnimationFrame(animationFrameRef.current);
    }
  }, [gameState.score, gameState.displaySpeed, gameState.gameStartTime, dispatch]);
